...
import { View, Text, ScrollView, Image as ReactImage, StyleSheet, TouchableOpacity, DeviceEventEmitter } from 'react-native';
import jwt_decode from 'jwt-decode'

class ProfileVerifyPhone extends Component {
  constructor(props) {
    super(props);
    this.state = {
      verifyCode: '',
  ...
  verification() {
    ...
    },
    {
      headers: {
        ...
        this.setState({temp_access_token: response.data.token})
        this.changePhone();
      }).catch((error) => {
        console.log(error);
        this.onVerifyFail();
  ...
  changePhone() {
    console.log("here")
    this.setState({error: '', loading: true});
    console.log(jwt_decode(this.props.jwt)._id)
    console.log(this.state.temp_access_token)
    axios.post('http://localhost:3000/api/user/updatePhone',{
      '_id': jwt_decode(this.props.jwt)._id
    },
    {
      headers: {
        'authorization': this.state.temp_access_token
      },
    }).then((response) => {
  ...
  cancel(){
    axios.post('http://localhost:3000/api/cancel',{
      ...
      this.props.authForgetPhoneGetVerify;
    }).catch((error) =>{
      // console.log(error);
      // this.onVerifyFail();
    });
