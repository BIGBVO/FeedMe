...
class AddPreference extends Component { 
  constructor(props) {
      ...
      this.searchPreference = this.searchPreference.bind(this);
  }
  ...
  searchPreference(){
    if (!this.state.search){
      alert("Please enter ingridient");
    } else {
      this.setState({loading: true})
      axios.get('http://localhost:3000/api/nutrition/search',{
        params: { keyword: this.state.search },
        headers: { authorization: this.props.jwt},
      }).then((response) => {
        if (response.data.results.length > 0){
          for (var i = 0; i < response.data.results.length; i++){
            this.state.search_list.push(response.data.results[i]);
          }
          this.setState({loading: false})
        } else {
          alert("No Ingridient with this keyword is available.")
          this.setState({loading: false})
        } 
      }).catch((error) => {
          console.log(error);
  ...
  render(){
    const {list, search, search_list} = this.state;
        ...
        return (
          <View style={styles.home}>
            ...
            <ScrollView style = {styles.MovieContainerGrid}>
              <Text onPress={this.props.home}  style={styles.home_edit}>
                Back
              </Text>
              ...
              <View>
                <TouchableOpacity onPress={this.searchPreference} style={styles.searchButton}>
                  <Text style={styles.search}>
                    Search
                  </Text>
                </TouchableOpacity>
              </View> 
              <FlatList style={styles.FlatlistStyles} data={list} numColumns={3} renderItem={renderList}
